...
void game_record_to_string(
    ...
    u32 idx = snprintf(buf, buf_siz, "White (%c): %s\nBlack (%c): %s\n", WHITE_STONE_CHAR, gr->white_name,
                                                                         BLACK_STONE_CHAR, gr->black_name);
    if (gr->finished) {
        if (gr->resignation) {
            idx += snprintf(buf + idx, buf_siz - idx, "Winner: %s by resignation\n",
                            gr->final_score > 0 ? gr->black_name : gr->white_name);
        } else if (gr->timeout) {
            idx += snprintf(buf + idx, buf_siz - idx, "Winner: %s by timeout\n",
                            gr->final_score > 0 ? gr->black_name : gr->white_name);
        } else {
    ...
    if (gr->handicap_stones.count > 0) {
        idx += snprintf(buf + idx, buf_siz - idx, "Handicap stones (%u):", gr->handicap_stones.count);
...
#endif
                if (gr->handicap_stones.count == 0) {
                    idx += snprintf(buf + idx, buf_siz - idx, " %c%s", (i & 1) == 0 ? 'B' : 'W', v);
                } else {
                    idx += snprintf(buf + idx, buf_siz - idx, " %c%s", (i & 1) == 1 ? 'B' : 'W', v);
                }
            } else {
                if (gr->handicap_stones.count == 0) {
                    idx += snprintf(buf + idx, buf_siz - idx, " %c--", (i & 1) == 0 ? 'B' : 'W');
                } else {
                    idx += snprintf(buf + idx, buf_siz - idx, " %c--", (i & 1) == 1 ? 'B' : 'W');
                }
            }
        }
...
move select_play(
    ...
    */
    ...
    if (best_play == PASS || gr->turns == 0 || !test_superko(gr, is_black, best_play)) {
        return best_play;
    }
