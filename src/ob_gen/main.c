...
static void export_table_as_ob(
    ...
    snprintf(str, MAX_PAGE_SIZ, "%s%ux%u.ob.new", data_folder(), BOARD_SIZ, BOARD_SIZ);

    FILE * fp = fopen(str, "w");
    if (fp == NULL) {
        fprintf(stderr, "error: failed to open opening book file for writing\n");
        release(str);
        exit(EXIT_FAILURE);
    }
    ...
    printf("Exported %u unique rules; %u were disqualified for not enough samples or majority representative\n", exported, skipped);
}
...
lbl_usage:
        ...
        printf("--min_game_turns number - Minimum number of turns for the game to be used. (default: %u)\n", minimum_turns);
        printf("--min_samples - Minimum number of samples for a rule to be saved. (default: %u)\n", minimum_samples);
    ...
    for (u32 fid = 0; fid < filenames_found; ++fid) {
        ...
        for (d16 k = 0; k < MIN(ob_depth, gr->turns); ++k) {
            ...
            if (entry == NULL) { /* new state */
                simple_state_transition * entry = malloc(sizeof(simple_state_transition));
                if (entry == NULL) {
