...
static void gtp_boardsize(
    ...
    if (ns != BOARD_SIZ) {
        ...
        fprintf(stderr, "board size cannot be changed on runtime; please edit the master header file and recompile matilda\n");
        char * s = alloc();
...
static void generic_genmove(
    ...
    */
    if (current_game.turns > 0 && current_player_color(&current_game) != is_black) {
        current_state.last_played = NONE;
        current_state.last_eaten = NONE;
    }

    u32 time_to_play = 0;
    time_system * curr_clock = is_black ? &current_clock_black : &current_clock_white;
    ...
    */
    ...
    if (has_play) {
        ...
        */
        ...
        snprintf(buf, MAX_PAGE_SIZ, "matilda playing as %s (%c) resigns\n",
                ...
                is_black ? BLACK_STONE_CHAR : WHITE_STONE_CHAR);
        flog_info("gtp", buf);
    ...
    if (commit_game_changes) {
        if (limit_by_playouts == 0) {
            u32 elapsed = (u32)(current_time_in_millis() - request_received_mark);
...
static void gtp_time_settings(
    ...
    set_time_system(&current_clock_black, new_main_time * 1000, new_byo_yomi_time * 1000, new_byo_yomi_stones, 1);
    set_time_system(&current_clock_white, new_main_time * 1000, new_byo_yomi_time * 1000, new_byo_yomi_stones, 1);
    ...
    if (strcmp(previous_ts_as_s, new_ts_as_s) == 0) {
        snprintf(buf, MAX_PAGE_SIZ, "clock settings kept at %s for both players", previous_ts_as_s);
    } else {
        snprintf(buf, MAX_PAGE_SIZ, "clock settings changed from %s to %s for both players", previous_ts_as_s, new_ts_as_s);
    }
...
static void gtp_kgs_time_settings(
    ...
    if (strcmp(systemstr, "none") == 0) {
    ...
    } else if (strcmp(systemstr, "absolute") == 0) {
        u32 new_main_time;
        if (main_time == NULL || !parse_uint(&new_main_time, main_time) || new_main_time > (UINT32_MAX / 1000)) {
            gtp_error(fp, id, "syntax error");
            release(previous_ts_as_s);
            return;
        }

        set_time_system(&current_clock_black, new_main_time * 1000, 0, 0, 0);
        set_time_system(&current_clock_white, new_main_time * 1000, 0, 0, 0);
    } else if (strcmp(systemstr, "byoyomi") == 0) {
        ...
        if (byo_yomi_time == NULL || !parse_uint(&new_byo_yomi_time, byo_yomi_time) || new_byo_yomi_time > (UINT32_MAX / 1000)) {
            gtp_error(fp, id, "syntax error");
            release(previous_ts_as_s);
            return;
        }

        if (byo_yomi_periods == NULL || !parse_uint(&new_byo_yomi_periods, byo_yomi_periods)) {
            gtp_error(fp, id, "syntax error");
            release(previous_ts_as_s);
            return;
        }

        set_time_system(&current_clock_black, new_main_time * 1000, new_byo_yomi_time * 1000, 1, new_byo_yomi_periods);
        set_time_system(&current_clock_white, new_main_time * 1000, new_byo_yomi_time * 1000, 1, new_byo_yomi_periods);
    } else if (strcmp(systemstr, "canadian") == 0) {
        u32 new_main_time;
        u32 new_byo_yomi_time;
        u32 new_byo_yomi_stones;
        ...
        if (byo_yomi_stones == NULL || !parse_uint(&new_byo_yomi_stones, byo_yomi_stones)) {
            gtp_error(fp, id, "syntax error");
            release(previous_ts_as_s);
            return;
        }
...
static void gtp_undo(
    ...
    if (number == NULL) {
        moves = 1;
    } else if (!parse_uint(&moves, number) || moves < 1) {
        gtp_error(fp, id, "syntax error");
        return;
    }
...
static void gtp_loadsgf(
    ...
    } else if (!parse_uint(&move_until, move_number) || move_until < 1) {
        gtp_error(fp, id, "syntax error");
        return;
    }
    ...
    while (1) {
        ...
        if (think_in_opt_turn) {
            do {
                ...
                } else if (ready == 0) { /* nothing to read */
...
lbl_parse_command:
        ...
        */
        ...
        if (argc == 0 && (strcmp(cmd, "list_commands") == 0 || strcmp(cmd, "help") == 0)) {
        ...
        if (argc > 1 && argc < 5 && strcmp(cmd, "kgs-time_settings") == 0) {
            gtp_kgs_time_settings(out_fp, idn, args[0], args[1], args[2], args[3]);
            continue;
        }
        ...
        if (command_exists) {
            fprintf(stderr, "warning: command '%s' exists but the parameter list is wrong; please check the documentation\n", cmd);
            ...
            if (best_dst_val < 4) {
                fprintf(stderr, "warning: command '%s' does not exist; did you mean '%s'?\n", cmd, best_dst_str);
            } else {
                fprintf(stderr, "warning: command '%s' does not exist; run \"help\" for a list of available commands\n", cmd);
            }
