            ...
            */
            for (move m = 0; m < TOTAL_BOARD_SIZ; ++m) {
                if (b.p[m] == EMPTY && m != b.last_eaten) {
                    ...
                    u8 l1 = libs_after_play_slow(&b, is_black, m, &stones_captured1);
                    move stones_captured2;
                    u8 l2 = libs_after_play(&cb, is_black, m, &stones_captured2);
                    ...
                    if (l1 != l2 || (l1 >= 2 && l3 != 2) || (l1 < 2 && l1 != l3)) {
                        ...
                        printf("tested %s playing (%s), l1=%u l2=%u l3*=%u l4*=%u\n", is_black ? "black" : "white", s, l1, l2, l3, l4);
                        fprint_cfg_board(stdout, &cb);
                        exit(EXIT_FAILURE);
                    }

                    if ((stones_captured3 && l3 == 0) || stones_captured1 !=
                        stones_captured2 || ((stones_captured1 > 0) != stones_captured3)) {
                        fprintf(stderr, "stones captured mismatch (1): (%u) %u %u %u\n", m, stones_captured1, stones_captured2, stones_captured3);
                        fprint_cfg_board(stdout, &cb);
                        exit(EXIT_FAILURE);
                    }
                }
            ...
            for (move m = 0; m < TOTAL_BOARD_SIZ; ++m) {
                if (b.p[m] == EMPTY && m != b.last_eaten) {
                    ...
                    if ((stones_captured3 && l3 == 0) || stones_captured1 !=
                        stones_captured2 || ((stones_captured1 > 0) != stones_captured3)) {
                        fprintf(stderr, "stones captured mismatch (2): (%u) %u %u %u\n", m, stones_captured1, stones_captured2, stones_captured3);
                        fprint_board(stdout, &b);
                        fprint_cfg_board(stdout, &cb);
                        exit(EXIT_FAILURE);
                    }
    ...
    for (u32 tries = 0; tries < 50; ++tries) {
        ...
        for (move ki = 0; ki < TOTAL_BOARD_SIZ; ++ki) {
            ...
            for (move m = 0; m < TOTAL_BOARD_SIZ; ++m) {
                ...
                massert(hash_cfg == hash_pat3, "CFG from play and pat3 patterns 1");

                cfg_board_clone(&sb2, &cb);
                hash_cfg = sb2.hash[m];
                massert(hash_cfg == hash_pat3, "CFG from board and pat3 patterns 2");
                cfg_board_free(&sb2);
            }
...
static void test_ladders() {
    ...
    */
    ...
    cfg_from_board(&cb, &b);
    massert(get_killing_play(&cb, cb.g[coord_to_move(1, 1)]) == coord_to_move(1, 2), "can_be_killed1");
    massert(get_saving_play(&cb, cb.g[coord_to_move(1, 1)]) == coord_to_move(1, 2), "can_be_saved1");
    cfg_board_free(&cb);
    ...
    /* Now it's a ladder */
    cfg_from_board(&cb, &b);
    massert(get_killing_play(&cb, cb.g[coord_to_move(1, 1)]) == coord_to_move(1, 2), "can_be_killed2");
    massert(get_saving_play(&cb, cb.g[coord_to_move(1, 1)]) == NONE, "can_be_saved2");
    cfg_board_free(&cb);

    b.p[coord_to_move(6, 5)] = BLACK_STONE;
    /* Still a ladder */
    cfg_from_board(&cb, &b);
    massert(get_killing_play(&cb, cb.g[coord_to_move(1, 1)]) == coord_to_move(1, 2), "can_be_killed3");
    massert(get_saving_play(&cb, cb.g[coord_to_move(1, 1)]) == NONE, "can_be_saved3");
    cfg_board_free(&cb);

    b.p[coord_to_move(6, 5)] = WHITE_STONE;
    /* No longer a ladder because white can connect */
    cfg_from_board(&cb, &b);
    massert(get_killing_play(&cb, cb.g[coord_to_move(1, 1)]) == coord_to_move(1, 2), "can_be_killed4");
    massert(get_saving_play(&cb, cb.g[coord_to_move(1, 1)]) == coord_to_move(1, 2), "can_be_saved4");
    cfg_board_free(&cb);
    ...
    */
    ...
    cfg_from_board(&cb, &b);
    massert(get_killing_play(&cb, cb.g[coord_to_move(3, 3)]) == coord_to_move(3, 4), "can_be_killed5");
    massert(get_saving_play(&cb, cb.g[coord_to_move(3, 3)]) == coord_to_move(3, 4), "can_be_saved5");
    cfg_board_free(&cb);
    ...
    */
    ...
    cfg_from_board(&cb, &b);
    massert(get_killing_play(&cb, cb.g[coord_to_move(3, 3)]) == coord_to_move(3, 4), "can_be_killed6");
    massert(get_saving_play(&cb, cb.g[coord_to_move(3, 3)]) == NONE, "can_be_saved6");
    cfg_board_free(&cb);
    ...
    */
    ...
    cfg_from_board(&cb, &b);
    massert(get_killing_play(&cb, cb.g[coord_to_move(0, 0)]) == coord_to_move(1, 2), "can_be_killed7");
    massert(get_saving_play(&cb, cb.g[coord_to_move(0, 0)]) == coord_to_move(1, 2), "can_be_saved7");
    cfg_board_free(&cb);
...
static void test_board() {
    ...
    for (u32 tes = 0; tes < 10000; ++tes) {
        ...
        massert(memcmp(b.p, b2.p, TOTAL_BOARD_SIZ) == 0, "packing/unpacking");
...
static void test_whole_game() {
    ...
    while (1) {
        ...
        bool has_play = evaluate_position_timed(&b, is_black, &out_b, stop_time, early_stop_time);
        if (!has_play) {
            break;
        }
